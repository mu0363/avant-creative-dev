...
export const StepNav = ({ check, stepNum }) => {
  return (
    <div>
      {check ? (
        <div className='grid place-items-center'>
          <CISolid className='h-6 sm:h-8 text-blue-400' />
          <p className='text-gray-400 text-xs sm:text-base'>{`Step${stepNum}`}</p>
        </div>
      ) : (
        <div className='grid place-items-center'>
          <CIOutline className='h-6 sm:h-8 text-gray-300' />
          <p className='text-gray-300 text-xs sm:text-base'>{`Step${stepNum}`}</p>
        </div>
      )}
    </div>
  );
